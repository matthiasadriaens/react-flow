...
import { MiniMapDragProps } from './types';

function MiniMapDrag({ x = 0, y = 0, width = 0, height = 0 }: MiniMapDragProps) {
  const dragRef = useRef<SVGRectElement>(null);
  const { getViewport, setViewport } = useReactFlow();

  useEffect(() => {
    if (dragRef.current) {
      const onDrag = (evt: D3DragEvent<SVGRectElement, null, SubjectPosition>) => {
  ...
  return <rect ref={dragRef} fill="rgba(255, 0,0,.5)" x={x} y={y} width={width} height={height} />;
}
